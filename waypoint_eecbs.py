    ...
    def _parse_scenarios(self) -> List[Dict]:
        ...
        for line in lines[1:]:
            ...
            try:
                ...
                waypoints = []
                if len(parts) > 10:
                    # The waypoints are in the remaining parts after the first 10
                    waypoint_coords = parts[10:]
...
class WaypointEECBSRunner:
    ...
    def run_waypoint_scenario(self, map_file: str, scenario_file: str, 
                             ...
                             timeout: int = 60, suboptimality: float = 5.0,
        ...
        for i, agent in enumerate(agents):
            ...
            print(f"    Full sequence: {[agent['start']] + agent['waypoints'] + [agent['goal']]}")
        ...
        # Verify waypoint sequence is followed
        print(f"\nVerifying waypoint sequences:")
        for agent_id in range(num_agents):
            if agent_id < len(complete_paths) and complete_paths[agent_id]:
                path = complete_paths[agent_id]
                expected_sequence = [agents[agent_id]['start']] + agents[agent_id]['waypoints'] + [agents[agent_id]['goal']]
                print(f"  Agent {agent_id}: Path length {len(path)}, Expected sequence: {expected_sequence}")
                if len(path) > 0:
...
def main():
    ...
    parser.add_argument('--suboptimality', type=float, default=5.0,
                       help='Suboptimality bound (default: 5.0)')
